  ...
  return (
    <div className="max-w-xs w-full border border-gray-200 shadow-lg m-3 flex flex-col overflow-hidden rounded-lg transition transform hover:-translate-y-1 hover:shadow-xl duration-300 bg-white">
      <div className="w-full h-72 overflow-hidden">
        ...
        <p className='bg-gradient-to-r from-green-600 via-green-500 to-green-400  fixed top-4 text-white p-1 w-36 text-xs font-semibold text-center'>BESTSELLER</p>}
        <img className="w-full h-full object-cover" src={product?.img[0]?.url || 'default-image-url'} alt={product.name} />
      </div>
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div className="mb-4">
          <h2 className="text-xl font-bold text-gray-900 mb-1 capitalize">{product.name}</h2>
          <p className="text-gray-600 text-sm  overflow-hidden overflow-ellipsis" style={{ height: '3rem' }}>
            {product.description}
          </p>
        </div>
